...
int main() {
	...
	while (true) {
		...
		std::cout << "1、长度（支持转换单位：m、km、mile、foot、inch、mm、cm、chi、cun、zhang)" << std::endl;
		std::cout<< "2、质量（支持转换单位：kg、g、mg、gr、lb、ct、t、oz、q、point)" << std::endl;
		std::cout<< "3、时间（支持转换单位：s、min、h、day、ms)" << std::endl;
		std::cout <<"4、速度（支持转换单位：长度/时间的任意组合)" << std::endl;
		std::cout<<"5、力（支持转换单位：kgf、gf、lbf、tf)"<<std::endl;
		std::cout<< "输入序号：";
		std::cin >> typeNum;
		...
		case 5:
			{
				Force f(data,unit);
				std::printf("结果是：%lf",f.convert(toUnit));
				std::cout<<toUnit<<std::endl;
				break;
			}
		}
